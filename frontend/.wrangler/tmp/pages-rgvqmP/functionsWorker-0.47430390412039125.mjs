...
// ../.wrangler/tmp/bundle-ZgmIy2/checked-fetch.js
...
function log(message2, data = {}) {
  console.log(`[AUTH] ${message2}`, JSON.stringify(data));
}
__name(log, "log");
async function onRequest({ request, env }) {
  ...
  try {
    ...
    if (payload.exp * 1e3 < Date.now()) {
      log("Token expired", { exp: payload.exp, now: Math.floor(Date.now() / 1e3) });
    ...
    const refreshExpiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1e3).toISOString();
    await env.DB.prepare(`
...
function log2(message2, data = {}) {
  console.log(`[LOGOUT] ${message2}`, JSON.stringify(data));
}
__name(log2, "log");
  ...
  if (env.DB) {
    ...
    if (refreshToken) {
      try {
        ...
        log2("Refresh token revoked in D1");
      } catch (error) {
        log2("Failed to revoke refresh token", { error: error.message });
      }
    }
  }
  const headers = new Headers();
...
function log3(message2, data = {}) {
  console.log(`[ME] ${message2}`, JSON.stringify(data));
}
__name(log3, "log");
async function onRequest5({ request, env }) {
  try {
    ...
    log3("Token", { hasToken: !!token, tokenPreview: token?.substring(0, 20) });
    if (!token) {
      return new Response(
    ...
    log3("Error", { message: error.message, name: error.name });
    return new Response(
...
function log4(message2, data = {}) {
  console.log(`[REFRESH] ${message2}`, JSON.stringify(data));
}
__name(log4, "log");
async function onRequest6({ request, env }) {
  try {
    ...
    const refreshToken = cookies.refresh_token;
    log4("Refresh attempt", { hasRefreshToken: !!refreshToken });
    if (!refreshToken) {
    ...
    if (!tokenResult) {
      log4("Refresh token not found");
      return new Response("Invalid refresh token", { status: 401 });
    }
    if (tokenResult.revoked_at) {
      log4("Refresh token revoked", { revokedAt: tokenResult.revoked_at });
      return new Response("Refresh token revoked", { status: 401 });
    }
    if (new Date(tokenResult.expires_at) < /* @__PURE__ */ new Date()) {
      log4("Refresh token expired", { expiresAt: tokenResult.expires_at });
      return new Response("Refresh token expired", { status: 401 });
    }
    log4("Refresh token valid", { userId: tokenResult.user_id, email: tokenResult.email });
    const newJwt = await signJwt(
      { sub: tokenResult.email, email: tokenResult.email, userId: tokenResult.user_id },
      env.JWT_SECRET
    );
    log4("New JWT signed", { length: newJwt.length });
    ...
    log4("Error", { message: error.message });
...
// ../.wrangler/tmp/pages-rgvqmP/functionsRoutes-0.7717230164815627.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-ZgmIy2/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-ZgmIy2/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.47430390412039125.mjs.map
