...
import { createSignal, createResource, createUniqueId, Show, For, createEffect } from "solid-js";
import { useMap } from "../context/MapContext";
import { 
  Search, MapPin, Navigation, Plus, Bookmark, 
  Trash2, X, Loader2, Crosshair, Check
} from "lucide-solid";
  ...
  async function fetchResults(query) {
    ...
    try {
      const response = await fetch(
        `${BACKEND_URL}/api/search?q=${encodeURIComponent(query)}&lon=${center[0]}&lat=${center[1]}&location_bias_scale=0.5`,
        { credentials: "include" }
      );
  ...
  return (
    <div class="space-y-6">
        ...
        <div class="relative">
          ...
          <Show when={getAutocomplete() && isSearchFocused()}>
            <div class="absolute inset-0 pl-10 pr-10 py-3 pointer-events-none">
              ...
              <span class="text-sm text-[var(--text-tertiary)]">{getAutocomplete().slice(query().length)}</span>
            </div>
          </Show>
          ...
          <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
            <Show 
              when={!isSearching()} 
              fallback={<Loader2 size={16} class="animate-spin text-[var(--text-tertiary)]" />}
            >
              <Search size={16} strokeWidth={1.5} class="text-[var(--text-tertiary)]" />
            </Show>
          </div>
          <input
            ref={inputRef}
                      ...
                      >
                        <div class="flex-1 px-4 py-2.5 flex items-center gap-3 min-w-0">
                          <MapPin 
                            ...
                            class="flex-shrink-0"
                            classList={{
                              "text-[var(--text-primary)]": selectedIndex() === index(),
                              "text-[var(--text-tertiary)]": selectedIndex() !== index(),
                            }}
                          />
                          <div class="flex-1 min-w-0 mr-3">
                            <p 
                              ...
                              classList={{
                                "text-[var(--text-primary)] font-medium": selectedIndex() === index(),
                                "text-[var(--text-secondary)]": selectedIndex() !== index(),
                              }}
                            >
                          ...
                          <div class="flex-shrink-0 flex items-center gap-1.5 text-[var(--text-tertiary)]">
                            <Navigation
                              size={11}
                              strokeWidth={2}
                              style={{ transform: `rotate(${getAngle(item.geometry.coordinates)}deg)` }}
                            />
              ...
              <Show when={results()?.length > 0}>
                <div class="px-4 py-2 text-xs text-[var(--text-tertiary)] flex items-center gap-3">
                  <span class="flex items-center gap-1">
                    <kbd class="px-1.5 py-0.5 bg-[var(--bg-secondary)] rounded text-[10px]">↑↓</kbd>
                    navigate
                  </span>
                  <span class="flex items-center gap-1">
                    <kbd class="px-1.5 py-0.5 bg-[var(--bg-secondary)] rounded text-[10px]">tab</kbd>
                    complete
                  </span>
                  <span class="flex items-center gap-1">
                    <kbd class="px-1.5 py-0.5 bg-[var(--bg-secondary)] rounded text-[10px]">↵</kbd>
      ...
      <div 
        ...
        classList={{
          "opacity-20 blur-sm pointer-events-none scale-[0.98]": isSearchActive(),
        }}
      >
        <LocationList />
      </div>
...
export function LocateMeButton() {
  ...
  return (
    <button
      ...
      disabled={isLocating()}
      data-locate-btn
      class="p-2 text-[var(--text-tertiary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-hover)] rounded-lg transition-all disabled:opacity-50"
        ...
        <Show 
          ...
          fallback={
            <div class="text-center py-12">
              <Bookmark size={20} strokeWidth={1.5} class="mx-auto mb-2 text-[var(--text-tertiary)]" />
          ...
          <ul class="space-y-0.5">
            <For each={locations()}>
              {(location, index) => (
                ...
                >
                  <div class="flex items-center gap-2 px-2 py-2 -mx-2 rounded-lg hover:bg-[var(--bg-hover)] transition-all cursor-pointer"
                    onClick={() => goToLocation(location)}
                  >
                    <MapPin size={14} strokeWidth={1.5} class="text-[var(--text-tertiary)] flex-shrink-0" />
...
export function AddLocationButton(props) {
  ...
  return (
    <>
      ...
      >
        <p class="text-sm font-medium text-[var(--text-primary)] mb-3">Save place</p>
        <div class="space-y-3">
          <input 
            type="text" 
            name="locationName" 
